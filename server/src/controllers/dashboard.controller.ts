...
export const getDashboardMetrics = async (req: Request, res: Response) => {
  try {
    ...
    // --- Inicio: Cálculo de Generaciones Activas Mensuales ---
    const monthlyActiveGenerations = await ContentGeneration.aggregate([
      {
        $match: {
          isDeleted: { $ne: true } // Considerar solo las no borradas
        }
      },
      {
        $group: {
    ...
    // --- Fin: Cálculo de Generaciones Activas Mensuales ---
    ...
    const metrics = {
      ...
      monthlyActiveGenerations // Nueva métrica de generaciones activas mensuales
    };
